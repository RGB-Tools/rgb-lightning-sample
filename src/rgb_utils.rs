...
use rgb::fungible::allocation::{OutpointValue, AllocatedValue};
use rgb::{Contract, ContractId, EndpointValueMap, InmemConsignment, TransferConsignment, AssignedState};
...
pub(crate) fn get_rgb_total_amount(contract_id: ContractId, rgb_node_client: Arc<Mutex<Client>>, wallet_arc: Arc<Mutex<Wallet<SqliteDatabase>>>) -> Result<u64, Error>  {
...
pub(crate) fn get_asset_owned_values(contract_id: ContractId, rgb_node_client: Arc<Mutex<Client>>, wallet_arc: Arc<Mutex<Wallet<SqliteDatabase>>>) -> Result<Vec<AssignedState<rgb::value::Revealed>>, Error>  {
	...
	sync_wallet(&wallet);
	let unspents_outpoints: Vec<OutPoint> = wallet.list_unspent().expect("valid unspent list")
		.iter().map(|u| u.outpoint).collect();
	Ok(contract_state.owned_values.into_iter()
		.filter(|ov| unspents_outpoints.contains(&OutPoint { txid: ov.seal.txid, vout: ov.seal.vout })).collect())
}

pub(crate) trait RgbUtilities {
	fn issue_contract(&mut self, amount: u64, outpoint: OutPoint, ticker: AsciiString, name: AsciiString, precision: u8)-> ContractId;

	fn send_rgb_internal(&mut self, contract_id: ContractId, psbt: &mut Psbt, input_outpoints_bt: BTreeSet<OutPoint>, beneficiaries: EndpointValueMap, change: Vec<AllocatedValue>) -> (PartiallySignedTransaction, InmemConsignment<TransferConsignment>);

	fn send_rgb(&mut self, contract_id: ContractId, psbt: PartiallySignedTransaction, input_outpoints_bt: BTreeSet<OutPoint>, beneficiaries: EndpointValueMap, change: Vec<AllocatedValue>) -> (PartiallySignedTransaction, InmemConsignment<TransferConsignment>);
}
...
impl RgbUtilities for Client {
	fn issue_contract(&mut self, amount: u64, outpoint: OutPoint, ticker: AsciiString, name: AsciiString, precision: u8)-> ContractId {
		...
		self
			.register_contract(asset.clone(), true, |_| ()).expect("valid contract");
	...
	fn send_rgb_internal(&mut self, contract_id: ContractId, psbt: &mut Psbt, input_outpoints_bt: BTreeSet<OutPoint>, beneficiaries: EndpointValueMap, change: Vec<AllocatedValue>) -> (PartiallySignedTransaction, InmemConsignment<TransferConsignment>) {
		...
		let transition = rgb_asset.transfer(input_outpoints_bt.clone(), beneficiaries.clone(), revealed_seal)
	...
	fn send_rgb(&mut self, contract_id: ContractId, psbt: PartiallySignedTransaction, input_outpoints_bt: BTreeSet<OutPoint>, beneficiaries: EndpointValueMap, change: Vec<AllocatedValue>) -> (PartiallySignedTransaction, InmemConsignment<TransferConsignment>) {
		let mut psbt = <Psbt as BitcoinDeserialize>::deserialize(&serialize(&psbt))
			.expect("valid bdk psbt");
